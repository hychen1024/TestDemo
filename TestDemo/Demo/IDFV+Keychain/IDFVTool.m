...
+ (NSString *)getIDFV {
    ...
    NSString * const KEY_USERNAME = @"com.yourcompanyname.idfvkey.username";
    NSString * const KEY_PASSWORD = @"com.yourcompanyname.idfvkey.password";
    
    //测试用 清除keychain中的内容
    //[IDFVTools delete:KEY_USERNAME_PASSWORD];
    
    //读取账号中保存的内容
    NSMutableDictionary *readUserDataDic = (NSMutableDictionary *)[self load:KEY_USERNAME];
    ...
    {//如果是第一次 肯定获取不到 这个时候就存储一个
        ...
        //进行存储 并返回这个数据
        [self save:KEY_USERNAME data:needSaveDataDic];
